defmodule SymphonyElixir.AppServerTest do
  ...
  test "app server downgrades known Codex model cache schema warnings" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-app-server-model-cache-warning-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      while IFS= read -r line; do
        ...
        case "$count" in
          ...
          4)
            printf '%s%s\\n' \\
              'WARN codex_models_manager::cache: failed to load model cache: ' \\
              'missing field `supports_reasoning_summaries` at line 1 column 42' >&2
            printf '%s\\n' '{"method":"turn/completed"}'
            exit 0
            ;;
          *)
      ...
      issue = %Issue{
        id: "issue-model-cache-warning",
        identifier: "MT-94",
        title: "Model cache warning",
        description: "Ensure known model cache schema drift stays out of warning telemetry",
        state: "In Progress",
      ...
      log =
        capture_log([level: :warning], fn ->
          assert {:ok, _result} =
                   AppServer.run(workspace, "Capture model cache warning", issue, on_message: on_message)
        end)

      assert_received {:app_server_message, %{event: :turn_completed}}
      refute_received {:app_server_message, %{event: :malformed}}
      refute log =~ "supports_reasoning_summaries"
      refute log =~ "Codex turn stream output"
    after
      File.rm_rf(test_root)
    end
  end
  ...
  test "app server launches over ssh for remote workers" do
    ...
    try do
      ...
      expected_turn_policy = %{
        ...
        "writableRoots" => [
          remote_workspace,
          Path.join(remote_workspace, ".git")
        ],
